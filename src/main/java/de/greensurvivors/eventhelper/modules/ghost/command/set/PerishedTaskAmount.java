...
public class PerishedTaskAmount extends AGameSubCmd {

    public PerishedTaskAmount(@NotNull EventHelper plugin, @Nullable Permission permission) {
        super(plugin, permission);
    }

    @Override
    public @NotNull Set<@NotNull String> getAliases() {
        return Set.of("perishedtaskamount", "perishedtasks", "deadtasks", "deadtaskamount");
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull GhostGame game, @NotNull LinkedList<@NotNull String> args) {
        if (hasPermission(sender)) {
            if (!args.isEmpty()) {
                try {
                    int newTaskAmount = Integer.parseUnsignedInt(args.getFirst());
                    game.getConfig().setPerishedTaskAmount(newTaskAmount);

                    plugin.getMessageManager().sendPrefixedLang(sender, GhostLangPath.MESSAGE_PREFIX, GhostLangPath.COMMAND_SET_PERISHED_PLAYER_TASK_AMOUNT_SUCCESS,
                        Placeholder.unparsed(SharedPlaceHolder.NUMBER.getKey(), String.valueOf(newTaskAmount)));
