...
public class AmountOfVexes extends AGameSubCmd {

    public AmountOfVexes(@NotNull EventHelper plugin, @Nullable Permission permission) {
        super(plugin, permission);
    }

    @Override
    public @NotNull Set<@NotNull String> getAliases() {
        return Set.of("amountofvexes", "vexamount");
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull GhostGame game, @NotNull LinkedList<@NotNull String> args) {
        if (!args.isEmpty()) {
            try {
                int newVexAmount = Integer.decode(args.getFirst());

                if (newVexAmount >= 1) {
                    game.getConfig().setAmountOfVexes(newVexAmount);

                    plugin.getMessageManager().sendPrefixedLang(sender,
                        GhostLangPath.MESSAGE_PREFIX, GhostLangPath.COMMAND_SET_VEX_AMOUNT_SUCCESS,
                        Placeholder.unparsed(SharedPlaceHolder.NUMBER.getKey(), String.valueOf(newVexAmount)));
                } else {
                    plugin.getMessageManager().sendPrefixedLang(sender,
                ...
                plugin.getComponentLogger().debug("could not decode Integer argument {} in command to set amount of vexes!", args.getFirst(), e);
