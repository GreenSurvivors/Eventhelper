...
public class StartingFood extends AGameSubCmd {

    public StartingFood(@NotNull EventHelper plugin, @Nullable Permission permission) {
        super(plugin, permission);
    }

    @Override
    public @NotNull Set<@NotNull String> getAliases() {
        return Set.of("startingfood");
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull GhostGame game, @NotNull LinkedList<@NotNull String> args) {
        if (!args.isEmpty()) {
            try {
                int startingFoodAmount = Integer.parseInt(args.getFirst());

                if (startingFoodAmount >= 0) {
                    game.getConfig().setStartingFoodAmount(startingFoodAmount);

                    plugin.getMessageManager().sendPrefixedLang(sender,
                        GhostLangPath.MESSAGE_PREFIX, GhostLangPath.COMMAND_SET_PLAYER_STARTING_FOOD_SUCCESS,
                        Placeholder.unparsed(SharedPlaceHolder.NUMBER.getKey(), String.valueOf(startingFoodAmount)));
                } else {
                    plugin.getMessageManager().sendPrefixedLang(sender,
                ...
                plugin.getComponentLogger().debug("could not decode int argument {} in command to set player start food amount!", args.getFirst(), e);
