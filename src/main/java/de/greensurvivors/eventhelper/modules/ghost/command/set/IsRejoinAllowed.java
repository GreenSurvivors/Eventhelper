...
public class IsRejoinAllowed extends AGameSubCmd {

    public IsRejoinAllowed(@NotNull EventHelper plugin, @Nullable Permission permission) {
        super(plugin, permission);
    }

    @Override
    public @NotNull Set<@NotNull String> getAliases() {
        return Set.of("rejoinallowed", "rejoin");
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull GhostGame game, @NotNull LinkedList<@NotNull String> args) {
        if (!args.isEmpty()) {
            ...
            if (isallowed != null) {
                game.getConfig().setIsRejoinAllowed(isallowed);

                plugin.getMessageManager().sendPrefixedLang(sender,
                    GhostLangPath.MESSAGE_PREFIX, GhostLangPath.COMMAND_SET_REJOIN_SUCCESS,
