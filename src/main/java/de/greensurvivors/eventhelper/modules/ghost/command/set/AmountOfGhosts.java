...
public class AmountOfGhosts extends AGameSubCmd {

    public AmountOfGhosts(@NotNull EventHelper plugin, @Nullable Permission permission) {
        super(plugin, permission);
    }

    @Override
    public @NotNull Set<@NotNull String> getAliases() {
        return Set.of("ghostamount");
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull GhostGame game, @NotNull LinkedList<@NotNull String> args) {
        if (!args.isEmpty()) {
            try {
                int newGhostAmount = Integer.decode(args.getFirst());
                ...
                plugin.getComponentLogger().debug("could not decode Integer argument {} in command to set amount of ghosts!", args.getFirst(), e);
