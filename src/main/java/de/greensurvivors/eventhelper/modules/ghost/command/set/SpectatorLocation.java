...
public class SpectatorLocation extends AGameSubCmd {

    public SpectatorLocation(@NotNull EventHelper plugin, @Nullable Permission permission) {
        super(plugin, permission);
    }

    @Override
    public @NotNull Set<@NotNull String> getAliases() {
        return Set.of("spectatorlocation");
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull GhostGame game, @NotNull LinkedList<@NotNull String> args) {
        if (sender instanceof Entity entity) {
            game.getConfig().setSpectatorStartLocation(entity.getLocation());

            plugin.getMessageManager().sendPrefixedLang(sender,
                GhostLangPath.MESSAGE_PREFIX, GhostLangPath.COMMAND_SET_SPECTATOR_LOCATION_SUCCESS);
        } else {
            plugin.getMessageManager().sendPrefixedLang(sender,
