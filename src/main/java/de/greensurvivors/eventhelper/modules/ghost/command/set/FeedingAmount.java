...
public class FeedingAmount extends AGameSubCmd {

    public FeedingAmount(@NotNull EventHelper plugin, @Nullable Permission permission) {
        super(plugin, permission);
    }

    @Override
    public @NotNull Set<@NotNull String> getAliases() {
        return Set.of("feedingamount");
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull GhostGame game, @NotNull LinkedList<@NotNull String> args) {
        if (!args.isEmpty()) {
            try {
                ...
                if (0 <= feedAmount) {
                    game.getConfig().setFeedAmount(feedAmount);

                    plugin.getMessageManager().sendPrefixedLang(sender,
                        GhostLangPath.MESSAGE_PREFIX, GhostLangPath.COMMAND_SET_PLAYER_FEEDING_AMOUNT_SUCCESS,
                ...
                plugin.getComponentLogger().debug("could not decode int argument {} in command to set player feeding amount!", args.getFirst(), e);
